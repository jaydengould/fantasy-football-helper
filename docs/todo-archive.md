# Outstanding work — CLOSED sections

Split out of `TODO.md` on 2026-09-02. Every section here is DONE, CLOSED,
SETTLED or FIXED; `TODO.md` keeps the queue and the sections still open.
Section numbers are unchanged, so a `§13` or `§18` reference anywhere in the
repo resolves here. Nothing reads this file to decide anything — it is the
reasoning and the measurements behind decisions already taken.

**Here:** §0 final review, §1 draft-night blockers, §2 (historical) survival,
§3 cheap fixes, §5-§7 Task 13 / draft_slot / league_id, §11 Task 13 defects,
§12 + §12a adp_source calibration, §13 ESPN, §16 cheat sheet, §17 crash
journal, §18 FantasyPros ECR, §20 Sleeper CDN, §21 calibrate.py.

**Still open, in `TODO.md`:** §8 Yahoo OAuth, §9 deferred minors, §10 later
phases, §14 bench ordering, §15 tier-not-ranking, §19 Phase 3.7.
...
## Phases 4 and 5 — the full closed record, moved here 2026-09-03

Everything below was in `TODO.md` until the doc cleanup. It is the
reasoning behind Phase 4a/4b/4c and Phase 5 and the sections that closed
with them. `§8`, `§9`, `§14`, `§15` and `§19` remain OPEN as items — their
one-line entries are in `TODO.md`, their analysis is here.
