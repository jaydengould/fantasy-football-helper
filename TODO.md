...
1. ~~**THE DRAFT DEBRIEF.**~~ **DONE 2026-09-01**, recorded in `CLAUDE.md`. Two
   ...
   Original framing kept below for why it mattered.
   ~~Do this first and it needs the user, not code.~~
...
3. **Phase 4 — season mode: start/sit + waivers.** Spec written 2026-09-01:
...
4. ~~**CONFIRM THE YAHOO STARTING SLOTS.**~~ **DONE 2026-09-01.** The user
...
5. **The Yahoo roster must be hand-entered** before season mode covers that
   league at all. No API, so nothing else can supply it.
6. **Phase 5 — trade finder.** Unblocked earlier than expected: every Sleeper
   roster is public and needs no auth. Own spec, after Phase 4.
7. **Phase 3.7 — the `DataTable` swap** (§19). Offseason. Carries a decision to
...
8. **Bench-mode ordering** (§14) and **tier-not-ranking awareness** (§15) are
...
9. **Deferred minors** (§9) — two left, both trivial, neither load-bearing.
10. **Task 1 (Yahoo OAuth)** — still no reply, 1–2 weeks quoted on 2026-08-24.
