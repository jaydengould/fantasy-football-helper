...
1. **Merge `phase-5-trade-finder`.** 500 tests, 204 mutations, merge-checked and
   awaiting the user. Agents never touch `main`.
2. **The weekly snapshot must keep running.** Nothing schedules it — a `lineup`
   ...
   both leagues.
3. **`.roster/yahoo-main.txt` must be updated after every Yahoo add/drop.** No
   API. `lineup` and `preflight` both print its age for this reason.
4. **Yahoo OAuth (Phase 0, §8) — blocked externally**, no reply since
...
5. **A run log.** The user reported the board "died at some point" during the live
...
7. **Leverage weighting** (§7c) — weight playoff weeks UP by win probability
...
8. **Win-probability lineups** (§7c) — optimise win probability (high floor when
   ...
   as item 7. **Gate: run `scripts/backtest_weekly.py` on 2024 AND 2025 first** —
...
9. **Bench-mode ordering** (§14). Once every starting slot is full the board says
...
10. **The top of every position is a TIER, not a ranking** (§15). Measured
...
11. From Phase 5 (§7d): two near-duplicate `last_scoring_week` fallback strings;
    ...
    cache-guarded).
12. From Phase 4a (§6): roster-file inline comments and duplicates (both fail
    ...
    active.
13. From Phase 1 (§9): `norm_name` strips one suffix, not in a loop (harmless for
    real names); `_stdin_reader`'s EOF warning path is never driven end-to-end.
14. **Cross-cutting, decide once or not at all:** print the trade/waiver FLOOR
...
| Event | Date | Note |
| --- | --- | --- |
| NFL week 1 | ~~Sept 9 2026~~ | season mode live — `lineup`, `waivers`, `trades` all shipped |
| Leverage-weighting build window | weeks 8–11 | item 7; the only dated piece of open work |
